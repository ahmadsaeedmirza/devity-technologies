...
const auditContent: Record<
  string,
  { headline: string; deliverables: { label: string; desc: string }[] }
> = {
  "web-platforms": {
    headline: "Find Out What's Actually Costing You Rankings and Conversions.",
    deliverables: [
      {
        label: "Technical SEO scan",
        desc: "Crawlability, indexing, structured data, and Core Web Vitals, checked against what's actually holding your rankings back",
      },
      {
        label: "Performance breakdown",
        desc: "Real load-time and rendering analysis, not just a generic score, tied to what's actually slow and why",
      },
      {
        label: "Prioritised recommendations",
        desc: "A ranked list of what to fix first, second, and third, based on effort versus impact",
      },
      {
        label: "A written report you keep",
        desc: "Delivered as a document, not just a call, so you have something concrete regardless of what you decide next",
      },
    ],
  },
  "saas-applications": {
    ...
    deliverables: [
      {
        ...
        desc: "Multi-tenancy, data isolation, and scaling readiness, checked against what happens at real usage volume",
      },
      {
        label: "Performance breakdown",
        desc: "Where load times, database queries, or infrastructure choices are costing you retention",
      },
      {
        label: "Prioritised recommendations",
        desc: "A ranked list of what to fix first, second, and third, based on effort versus impact",
    ...
    deliverables: [
      {
        ...
        desc: "Frame rates, crash reports, and load handling, checked against what's actually driving uninstalls",
      },
      {
        label: "Offline and sync review",
        desc: "Whether your app's offline handling and background sync hold up under real conditions",
      },
      {
        label: "Prioritised recommendations",
        desc: "A ranked list of what to fix first, second, and third, based on effort versus impact",
  ...
  "ai-automation": {
    headline:
      "Find Out Where AI Automation Could Actually Save You Time and Money.",
    deliverables: [
      {
        label: "Workflow bottleneck scan",
        desc: "Where manual work is actually costing you the most time, checked against what's realistically automatable",
      },
      {
        label: "Build vs no-code assessment",
        desc: "An honest read on whether your use case needs custom engineering or a simpler no-code tool would do",
      },
      {
        label: "Prioritised recommendations",
        desc: "A ranked list of what to automate first, second, and third, based on effort versus impact",
      },
      {
        label: "A written report you keep",
        desc: "Delivered as a document, not just a call, so you have something concrete regardless of what you decide next",
...
const faqs = [
  {
    ...
    a: "It is genuinely free. You'll get a real, written audit whether or not you ever work with us. There's no obligation and no hidden follow-up fee.",
  },
  {
    q: "Will I get pushed into a sales call?",
    a: "No. You'll receive the audit as a written report. If you have questions about it, you're welcome to ask, but there's no required call to receive your results.",
  },
  {
    q: "What if things are already in good shape?",
    a: "Then the audit will say so, and you'll know exactly what's already working. A short, honest report confirming things are fine is still useful, and costs you nothing to find out.",
  },
  {
    q: "How long does the audit take?",
    a: "You'll receive your written report within 48 hours of submitting your details.",
  },
  {
    q: "Do I need to be an existing or upcoming client to request this?",
    a: "No. This is open to anyone who wants a genuine, no-obligation look at their product's technical health.",
  },
];
...
const FreeTechnicalAudit = ({
  ...
  const [service, setService] = useState(
    initialService && auditContent[initialService]
      ? initialService
      : "web-platforms",
  );
        ...
        <div className="container grid sm:grid-cols-3 gap-8 text-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="font-mono text-[10px] tracking-[0.2em] uppercase text-background/60">
              Turnaround on your written report
            </p>
          </div>
          <div className="flex flex-col items-center gap-3">
